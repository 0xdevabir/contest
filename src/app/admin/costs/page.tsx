...
export default async function AdminCostsPage() {
  const scale = await estimateCurrentScale();
  ...
  return (
    <div className="mx-auto max-w-[1300px] px-4 py-7 sm:px-6 lg:px-8">
      ...
      <p className="mt-5 text-[11px] text-[var(--muted)]">
        Figures are the static Appendix I model, not live billing data — no
        billing API is wired up for any provider. The dominant scaling cost
        is judge CPU, which is lumpy (a midterm needs capacity for ten
        minutes a week); see docs/RUNBOOK.md for manual pre-scaling and
        `src/lib/autoscale.ts` for queue-depth autoscaling.
      </p>
    </div>
  );
}
