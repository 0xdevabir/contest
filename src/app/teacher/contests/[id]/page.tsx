...
import { CloneContest } from "@/components/contest/CloneContest";

type Props = { params: Promise<{ id: string }> };

export default async function TeacherContestControlPage({ params }: Props) {
  ...
  if (!caps.has("view")) notFound();

  const accepted = await prisma.submission.count({ where: { contestId: id, verdict: "AC" } });
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-4 py-7 sm:px-6 lg:px-8">
      ...
      <header className="mt-4 flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-[10px] text-[var(--muted)]">/{contest.slug}</span>
            <span className="font-mono text-[10px] text-[var(--muted)]">{contest.visibility}</span>
          </div>
        ...
        <div className="flex items-center gap-2">
          <Link href={`/contests/${contest.slug}`} target="_blank" className="btn btn-ghost !py-2 !text-xs">
            <ExternalLink size={13} aria-hidden="true" />
            Public page
          </Link>
          <CloneContest contestId={contest.id} />
          {caps.has("edit") && (
            <AdminContestActions
              contestId={contest.id}
              status={contest.status}
              durationMinutes={contest.durationMinutes}
              apiBase="/api/teacher/contests"
            />
          )}
        </div>
      </header>
      ...
      <section className="mt-6 grid gap-3 sm:grid-cols-3">
        <ControlMetric icon={Users} label="Registrations" value={contest._count.registrations} />
        <ControlMetric icon={Braces} label="Submissions" value={contest._count.submissions} />
      ...
      <div className="mt-6">
        {caps.has("edit") ? (
          ...
          <p className="panel p-5 text-sm text-[var(--muted)]">
            You can view this contest but not edit it — ask an owner or coauthor
            for edit access.
          </p>
        )}
      </div>
...
function ControlMetric({ icon: Icon, label, value }: { icon: typeof Users; label: string; value: number }) {
  return (
