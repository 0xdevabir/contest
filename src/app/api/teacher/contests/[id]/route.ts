...
import { toResponse, ValidationError, NotFoundError, ForbiddenError } from "@/lib/errors";
...
export async function GET(_req: Request, { params }: Params) {
  try {
    ...
    const contest = await prisma.contest.findUnique({
      where: { id },
      include: { problems: { orderBy: { order: "asc" } }, staff: true, _count: { select: { registrations: true, submissions: true } } },
    });
    ...
    return NextResponse.json({ ok: true, contest, capabilities: [...caps] });
  } catch (err) {
    return toResponse(err);
  }
}

/** Teacher-scoped edit — same transaction body as `/api/admin/contests/[id]`,
 * capability-gated instead of admin-only (docs/phases/PHASE-05-contest-engine.md
 * Stage 6). */
export async function PATCH(req: Request, { params }: Params) {
  try {
    ...
    const caps = await contestCapabilities(session, existing);
    ...
    let startsAt = data.startsAt !== undefined ? (data.startsAt ? new Date(data.startsAt) : null) : existing.startsAt;
    let endsAt = data.endsAt !== undefined ? (data.endsAt ? new Date(data.endsAt) : null) : existing.endsAt;
    ...
    if (data.action === "go-live") {
      ...
      status = "SCHEDULED";
      if (!startsAt) throw new ValidationError("startsAt required to schedule");
    }
    if (startsAt && endsAt && endsAt <= startsAt) throw new ValidationError("End time must be after start time");

    const rules = data.rules ? { ...defaultContestRules, ...(existing.rules as object), ...data.rules } : undefined;
